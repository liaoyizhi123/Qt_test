...
#                     run = rec.get("run", "")
#                     seg = rec.get("segment", "")
#                     f.write(f"{t0:.6f},{t1:.6f},{cond},duration={dur},run={run},segment={seg}\n")
...
class Page9Widget(QWidget):
    ...
    记录方式：
    - 支持 Leichi / Neuracle 设备选择
    - 实验结束后，Page9 读取 triggers.csv（Leichi 模式），对齐每个激活段的 start/end，
      并生成 txt + meta.json。
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("闭眼 / 心算 实验")
        self.setMinimumSize(900, 700)

        # ===== trigger code 定义 =====
        self.TRIG_CLOSED_START = 1
        self.TRIG_CLOSED_END = 2
        self.TRIG_MATH_START = 3
        self.TRIG_MATH_END = 4
        ...
        # ---------- 固定/默认参数 ----------
        self.initial_countdown = 10  # 实验开始前倒计时（秒）
        self.math_prep_countdown = 10  # 闭眼结束后，心算前的过渡倒计时（秒）
        self.end_countdown = 10  # 实验结束倒计时缓冲（秒）

        self.default_closed_duration = 60  # 默认闭眼时间（秒）
        self.default_math_duration = 60  # 默认心算时间（秒）
        self.default_rest_duration = 10  # 默认 run 间休息（秒）
        self.default_runs = 5  # 默认 run 数量

        # 条件标签
        self.COND_CLOSED = "closed_eyes"
        self.COND_MATH = "mental_math"
        ...
        self.closed_duration = self.default_closed_duration
        self.math_duration = self.default_math_duration
        self.rest_duration = self.default_rest_duration
    ...
    def _build_main_ui(self):
        ...
        self.instruction = QLabel(
            "填写信息后点击开始。\n"
            "本范式在每个 Run 内按顺序进行：闭眼休息 → 心算任务。\n"
            "如果是 Leichi 模式，系统将自动调用 Page2 开始保存数据。"
        )
        ...
        self.runs_spin.setRange(1, 999)
        self.runs_spin.setValue(self.default_runs)
        form.addRow("Runs 数:", self.runs_spin)

        self.closed_spin = QSpinBox()
        self.closed_spin.setRange(1, 600)
        self.closed_spin.setValue(self.default_closed_duration)
        form.addRow("闭眼时间 (秒):", self.closed_spin)

        self.math_spin = QSpinBox()
        self.math_spin.setRange(1, 600)
        self.math_spin.setValue(self.default_math_duration)
        form.addRow("心算时间 (秒):", self.math_spin)

        self.rest_spin = QSpinBox()
        self.rest_spin.setRange(0, 600)
        self.rest_spin.setValue(self.default_rest_duration)
        form.addRow("Run 间休息 (秒):", self.rest_spin)
    ...
    def _build_screen_ui(self):
        ...
        screen_layout.setAlignment(QtCore.Qt.AlignmentFlag.AlignCenter)

        # 大显示区 (用于只显示文字，不倒计时的情况)
        ...
        self.stage_label.setAlignment(QtCore.Qt.AlignmentFlag.AlignCenter)
        self.stage_label.hide()
        screen_layout.addWidget(self.stage_label)
        ...
        self.countdown_label.setAlignment(QtCore.Qt.AlignmentFlag.AlignCenter)
        self.countdown_label.hide()
        screen_layout.addWidget(self.countdown_label)
    ...
    def on_start_clicked(self):
        ...
        Tc = self.closed_spin.value()
        Tm = self.math_spin.value()
        Tr = self.rest_spin.value()
        runs = self.runs_spin.value()

        if Tc <= 0 or Tm <= 0:
            QMessageBox.warning(self, "错误", "闭眼或心算时间必须大于 0 秒。")
            return
        ...
        if self.device_type == "Leichi":
            eeg_page = getattr(self, "eeg_page", None)
            try:
                if eeg_page and hasattr(eeg_page, "start_saving"):
                    eeg_page.start_saving(self.run_dir)
            except Exception:
                pass
    ...
    def _start_run(self):
        ...
        # 语音提示与打码
        self._speak("请闭眼")
        self._send_trigger(self.TRIG_CLOSED_START)

        # 界面显示
        self._apply_bg("#ffffff")
        self._apply_fg("#000000")
        self.stage_label.setText("闭眼数据采集中")
        self.stage_label.show()
        self.countdown_label.hide()

        # 闭眼持续指定时间后，跳转到心算过渡
        QtCore.QTimer.singleShot(dur * 1000, self._end_closed_start_math_countdown)

    def _end_closed_start_math_countdown(self):
        # 闭眼阶段结束打码与语音提示
        self._send_trigger(self.TRIG_CLOSED_END)
        self._speak("请睁眼")
        ...
        self._show_fullscreen_message(
            "接下来将进行心算任务\n{n}秒后开始",
            self.math_prep_countdown,
            bg="#ffec99",
            fg="#000000",
            plain=False,
            next_callback=self._start_math,
        )

    def _start_math(self):
        # 过渡倒计时结束，时间推进
        self.logical_ms += self.math_prep_countdown * 1000

        dur = self.math_duration
        start_ms = self.logical_ms
        end_ms = start_ms + dur * 1000
        ...
        # 心算阶段开始打码
        self._send_trigger(self.TRIG_MATH_START)

        # 界面显示：不要倒计时，只显示静态文字，且无语音提示
        self._apply_bg("#ffffff")
        self._apply_fg("#000000")
        self.stage_label.setText("请从100开始，依次减去3，一直减下去")
        self.stage_label.show()
        self.countdown_label.hide()

        # 心算持续指定时间后结束
        QtCore.QTimer.singleShot(dur * 1000, self._end_math)

    def _end_math(self):
        # 心算阶段结束打码
        self._send_trigger(self.TRIG_MATH_END)
        ...
        # 检查是否还有下一个 run
        if self.current_run >= self.total_runs:
            # 修改：新增实验结束 10 秒倒计时，既给被试提示，也为写入 triggers.csv 争取缓冲时间
            self._show_fullscreen_message(
                "实验结束\n{n}秒后自动保存并退出",
                self.end_countdown,
                bg="#e6ffea",
    ...
    def _apply_bg(self, color: str):
        self.stage_label.setStyleSheet(
            f"background-color:{color}; padding: 18px 36px; border-radius: 8px;"
        )
        self.countdown_label.setStyleSheet(
            f"background-color:{color}; padding: 18px 36px; border-radius: 8px;"
        )
    ...
    def _apply_fg(self, color: str):
        self.stage_label.setStyleSheet(self.stage_label.styleSheet() + f"color:{color};")
        self.countdown_label.setStyleSheet(self.countdown_label.styleSheet() + f"color:{color};")

    def _clear_styles(self):
        self.stage_label.setStyleSheet("color: black;")
    ...
    def _parse_segments_from_triggers(self) -> list[dict]:
        ...
        start_codes = {self.TRIG_CLOSED_START, self.TRIG_MATH_START}
        end_codes = {self.TRIG_CLOSED_END, self.TRIG_MATH_END}
    ...
    def _write_report(self, aborted: bool = False):
        ...
        txt_path = os.path.join(
            base_dir,
            f"ClosedMath_{name}_{ts_for_name}_runs{self.total_runs}_{flag}.txt",
        )
        ...
        timing = {
            "initial_countdown": int(self.initial_countdown),
            "math_prep_countdown": int(self.math_prep_countdown),
            "closed_duration": float(self.closed_duration),
            "math_duration": float(self.math_duration),
            "rest_duration": float(self.rest_duration),
        }

        trigger_code_labels = {
            "0": "baseline",
            str(self.TRIG_CLOSED_START): "closed_eyes_start",
            str(self.TRIG_CLOSED_END): "closed_eyes_end",
            str(self.TRIG_MATH_START): "mental_math_start",
            str(self.TRIG_MATH_END): "mental_math_end",
        }
        ...
        meta_path = os.path.join(
            base_dir,
            f"ClosedMath_{name}_{ts_for_name}_meta.json",
        )
