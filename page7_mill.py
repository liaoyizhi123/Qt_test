...
#         # 想象阶段时长区间
#         self.imagine_min = 4.0
#         self.imagine_max = 5.0

#         self.assess_duration = 0.0
#         self.break_duration = 2.0
#         self.end_countdown = 10
#         self.default_trials = 16
...
#         # 新增：统一刺激区域
#         self.stimulus_wrapper: QWidget | None = None
#         self.stimulus_stack: QtWidgets.QStackedLayout | None = None

#         self.trigger_mapping: dict[str, dict[str, int]] = {
#             "静止": {"task_start": 1, "task_end": 2},
#             "慢走": {"task_start": 3, "task_end": 4},
#             "慢跑": {"task_start": 5, "task_end": 6},
#             "快跑": {"task_start": 7, "task_end": 8},
#         }
...
#         self._current_bg: str | None = None
...
#         self.trials_spin.setSingleStep(4)
...
#         form.addRow("视频引导时长 (秒):", task_widget)

#         # 想象阶段时长区间
#         imagine_widget = QtWidgets.QWidget(self.settings_widget)
#         imagine_box = QHBoxLayout(imagine_widget)
#         imagine_box.setContentsMargins(0, 0, 0, 0)
#         imagine_box.setSpacing(8)

#         self.imagine_min_spin = QSpinBox()
#         self.imagine_min_spin.setRange(1, 120)
#         self.imagine_min_spin.setSingleStep(1)
#         self.imagine_min_spin.setValue(int(self.imagine_min))

#         self.imagine_max_spin = QSpinBox()
#         self.imagine_max_spin.setRange(1, 120)
#         self.imagine_max_spin.setSingleStep(1)
#         self.imagine_max_spin.setValue(int(self.imagine_max))

#         imagine_box.addWidget(self.imagine_min_spin)
#         imagine_box.addWidget(self.imagine_max_spin)
#         form.addRow("想象阶段时长 (秒):", imagine_widget)
...
#         # ===== 共用刺激区域：视频和 + 占同一块固定空间 =====
#         self.stimulus_wrapper = QWidget(self.screen_container)
#         self.stimulus_wrapper.setFixedSize(960, 540)
#         self.stimulus_wrapper.setSizePolicy(
#             QSizePolicy.Policy.Fixed,
#             QSizePolicy.Policy.Fixed,
#         )

#         stimulus_layout = QVBoxLayout(self.stimulus_wrapper)
#         stimulus_layout.setContentsMargins(0, 0, 0, 0)
#         stimulus_layout.setSpacing(0)

#         self.video_widget = QVideoWidget(self.stimulus_wrapper)
...
#         self.cross_label = QLabel("+", self.stimulus_wrapper)
...
#         self.cross_label.setSizePolicy(
#             QSizePolicy.Policy.Expanding,
#             QSizePolicy.Policy.Expanding,
#         )

#         self.stimulus_stack = QtWidgets.QStackedLayout()
#         self.stimulus_stack.setContentsMargins(0, 0, 0, 0)
#         self.stimulus_stack.setStackingMode(QtWidgets.QStackedLayout.StackingMode.StackOne)
#         self.stimulus_stack.addWidget(self.video_widget)  # index 0
#         self.stimulus_stack.addWidget(self.cross_label)  # index 1

#         stimulus_layout.addLayout(self.stimulus_stack)
#         screen_layout.addWidget(
#             self.stimulus_wrapper, alignment=QtCore.Qt.AlignmentFlag.AlignCenter
#         )
...
#         screen_layout.addWidget(
#             self.task_count_label, alignment=QtCore.Qt.AlignmentFlag.AlignCenter
#         )
...
#                 QMessageBox.warning(
#                     self, "错误", "未找到 EEG 采集页面，请在主程序中确保已创建并注入 Page2Widget。"
#                 )
#                 return
...
#             else:
#                 QMessageBox.warning(
#                     self, "错误", "当前操作系统未配置 Neuracle 串口名称，请改用 Leichi 或修改代码。"
#                 )
#                 return
...
#                     self, "错误", f"无法导入 Neuracle 库 neuracle_lib.triggerBox：{e}"
#                 )
#                 return
...
#                 self.neuracle_trigger = None
#                 QMessageBox.critical(
#                     self, "错误", "Neuracle 串口连接失败，请连接 TriggerBox 并检查串口名称。"
#                 )
#                 return
...
#                 self.neuracle_trigger = None
#                 QMessageBox.critical(
#                     self, "错误", "Neuracle 串口无效，请检查连接（Invalid Serial!）。"
#                 )
#                 return
...
#         imin = int(self.imagine_min_spin.value())
#         imax = int(self.imagine_max_spin.value())
#         ta = int(self.assess_spin.value())
#         tb = int(self.break_spin.value())

#         if tmin > tmax:
#             QMessageBox.warning(self, "错误", "视频引导时长区间无效：最小值不能大于最大值。")
#             return

#         if imin > imax:
#             QMessageBox.warning(self, "错误", "想象阶段时长区间无效：最小值不能大于最大值。")
#             return

#         self.prompt_duration = p
#         self.task_min = tmin
#         self.task_max = tmax
#         self.imagine_min = imin
#         self.imagine_max = imax
...
#         task_sec = int(
#             random.randint(
#                 int(min(self.task_min, self.task_max)), int(max(self.task_min, self.task_max))
#             )
#         )
...
#         imagine_sec = int(
#             random.randint(
#                 int(min(self.imagine_min, self.imagine_max)),
#                 int(max(self.imagine_min, self.imagine_max)),
#             )
#         )
#         imagine_ms = imagine_sec * 1000
...
#         imagine_start_ms = task_end_ms
#         imagine_end_ms = imagine_start_ms + imagine_ms
...
#             "task_end_time": None,
#             "imagine_start_ms": imagine_start_ms,
#             "imagine_end_ms": imagine_end_ms,
#             "durations": {
#                 "prompt": int(self.prompt_duration),
#                 "task": int(task_sec),
#                 "imagine": int(imagine_sec),
...
#             "total_ms": prompt_ms + task_ms + imagine_ms + assess_ms + break_ms,
#         }
...
#             bg="#ffffff",
#             fg="#000000",
#             next_stage=self._stage_imagine,
#             show_cross=False,
#         )

#     def _stage_imagine(self):
#         self._stop_task_timer()
#         self._stop_task_video()
...
#         self._show_cross_stimulus()

#         dur = int(self.trial_logs[-1]["durations"]["imagine"])
...
#         if self.media_player is None or self.video_widget is None or self.stimulus_stack is None:
#             return
...
#         self.cross_label.hide()
#         self.stimulus_wrapper.show()
#         self.stimulus_stack.setCurrentIndex(0)

#         url = QtCore.QUrl.fromLocalFile(abs_path)
#         self.media_player.setSource(url)
#         self.media_player.play()

#     def _show_cross_stimulus(self):
#         if self.stimulus_stack is None:
#             return
#         self.cross_label.show()
#         self.video_widget.hide()
#         self.stimulus_wrapper.show()
#         self.stimulus_stack.setCurrentIndex(1)
...
#             self.video_widget.hide()
#         if self.stimulus_wrapper is not None:
#             self.stimulus_wrapper.hide()
...
#         except Exception as e:
#             QMessageBox.warning(
#                 self, "触发文件读取失败", f"读取 {self.triggers_filename} 失败：{e}"
#             )
#             return
...
#         mode = "start_end" if n_events >= 2 * n_trials else "start_only"
...
#             style = (
#                 f"color:{fg_color};background-color:{bg};padding: 12px 28px; border-radius: 8px;"
#             )
...
#         fname = os.path.join(
#             base_dir, f"EEGMILL_{name}_{ts_for_name}_trials{len(self.trial_logs)}_{flag}.txt"
#         )

#         cn_to_en_map = {"静止": "rest", "慢走": "walking", "慢跑": "jogging", "快跑": "sprint"}
...
#                     t0 = float(start_t) if isinstance(start_t, (int, float)) else float('nan')
#                     t1 = float(end_t) if isinstance(end_t, (int, float)) else float('nan')
...
#                     detail = (
#                         f"prompt={d['prompt']}|task={d['task']}|imagine={d.get('imagine', 0)}|"
...
#             "imagine_min": int(self.imagine_min),
#             "imagine_max": int(self.imagine_max),
...
#                 else None
#             )
#             t1 = float(end_t) if isinstance(end_t, (int, float)) and not math.isnan(end_t) else None
...
class Page7Widget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.trigger_code_labels: dict[int, str] = {
            ...
            10: "rest_task_start",
            11: "rest_task_end",
            12: "rest_imagine_start",
            13: "rest_imagine_end",
            20: "walking_task_start",
            21: "walking_task_end",
            22: "walking_imagine_start",
            23: "walking_imagine_end",
            30: "jogging_task_start",
            31: "jogging_task_end",
            32: "jogging_imagine_start",
            33: "jogging_imagine_end",
            40: "sprint_task_start",
            41: "sprint_task_end",
            42: "sprint_imagine_start",
            43: "sprint_imagine_end",
        }
    ...
    def _start_next_trial(self):
        ...
        log = {
            ...
            "imagine_start_ms": imagine_start_ms,
            "imagine_end_ms": imagine_end_ms,
            "imagine_start_time": None,
            "imagine_end_time": None,
            "durations": {
                "prompt": int(self.prompt_duration),
                "task": int(task_sec),
                "imagine": int(imagine_sec),
    ...
    def _stage_imagine(self):
        ...
        self._send_trigger_for_current_trial(stage="imagine_start")
        self._show_cross_stimulus()
    ...
    def _enter_assess(self):
        self._send_trigger_for_current_trial(stage="imagine_end")
    ...
    def _update_trial_times_from_triggers(self):
        ...
        n_events = len(events)
        if n_events >= 4 * n_trials:
            mode = "task_imagine_start_end"
        elif n_events >= 2 * n_trials:
            mode = "task_start_end"
        else:
            mode = "task_start_only"
        self.trigger_assignment_mode = mode

        for i, rec in enumerate(self.trial_logs):
            task_start_t = float("nan")
            task_end_t = float("nan")
            imagine_start_t = float("nan")
            imagine_end_t = float("nan")

            if mode == "task_imagine_start_end":
                idx0 = 4 * i
                idx1 = 4 * i + 1
                idx2 = 4 * i + 2
                idx3 = 4 * i + 3
                ...
                if idx0 < n_events:
                    task_start_t = events[idx0][0]
                if idx1 < n_events:
                    task_end_t = events[idx1][0]
                elif not math.isnan(task_start_t):
                    task_end_t = task_start_t + float(rec["durations"]["task"])

                if idx2 < n_events:
                    imagine_start_t = events[idx2][0]
                elif not math.isnan(task_end_t):
                    imagine_start_t = task_end_t

                if idx3 < n_events:
                    imagine_end_t = events[idx3][0]
                elif not math.isnan(imagine_start_t):
                    imagine_end_t = imagine_start_t + float(rec["durations"]["imagine"])

            elif mode == "task_start_end":
                idx_start = 2 * i
                idx_end = 2 * i + 1
                if idx_start < n_events:
                    task_start_t = events[idx_start][0]
                    task_end_t = task_start_t + float(rec["durations"]["task"])
                    imagine_start_t = task_end_t
                    imagine_end_t = imagine_start_t + float(rec["durations"]["imagine"])
                if idx_end < n_events:
                    imagine_end_t = events[idx_end][0]
            else:
                if i < n_events:
                    task_start_t = events[i][0]
                    task_end_t = task_start_t + float(rec["durations"]["task"])
                    imagine_start_t = task_end_t
                    imagine_end_t = imagine_start_t + float(rec["durations"]["imagine"])

            rec["task_start_time"] = task_start_t
            rec["task_end_time"] = task_end_t
            rec["imagine_start_time"] = imagine_start_t
            rec["imagine_end_time"] = imagine_end_t

            for t in (task_start_t, task_end_t, imagine_start_t, imagine_end_t):
                if not math.isnan(t):
                    if self.eeg_exp_start is None or t < self.eeg_exp_start:
                        self.eeg_exp_start = t
                    if self.eeg_exp_end is None or t > self.eeg_exp_end:
                        self.eeg_exp_end = t
    ...
    def _save_report(self, aborted: bool = False):
        ...
        try:
            with open(fname, 'w', encoding='utf-8') as f:
                for rec in self.trial_logs:
                    start_t = rec.get("task_start_time")
                    end_t = rec.get("imagine_end_time")
    ...
    def _save_meta_json(self, aborted: bool = False):
        ...
        trials_json: list[dict] = []
        for rec in self.trial_logs:
            task_start_t = rec.get("task_start_time")
            task_end_t = rec.get("task_end_time")
            imagine_start_t = rec.get("imagine_start_time")
            imagine_end_t = rec.get("imagine_end_time")
            t0 = (
                float(task_start_t)
                if isinstance(task_start_t, (int, float)) and not math.isnan(task_start_t)
                else None
            )
            t1 = (
                float(task_end_t)
                if isinstance(task_end_t, (int, float)) and not math.isnan(task_end_t)
                else None
            )
            t2 = (
                float(imagine_start_t)
                if isinstance(imagine_start_t, (int, float)) and not math.isnan(imagine_start_t)
                else None
            )
            t3 = (
                float(imagine_end_t)
                if isinstance(imagine_end_t, (int, float)) and not math.isnan(imagine_end_t)
                else None
            )
