...
class Page8Widget(QWidget):  # 这里没有run的概念
    ...
    现在的时间记录逻辑：
    - 点击“开始实验”后，立刻调用 Page2 的 start_saving() 开始写 4 个 CSV。
    - 每个 trial 的 Task 阶段：
        * Task 开始时，从 Page2 获取片上时间，记为 task_start_hw（秒）。
        * Task 结束、进入自评前，再获取片上时间，记为 task_end_hw（秒）。
      这两个时间写入 txt 每行的前两列，可直接和 CSV 中的 Time 列对齐截取。
    - 所有 trial 完成后，还有一段“X 秒后实验结束”的倒计时，等倒计时结束后，
      调用 Page2 的 stop_saving()，然后写 txt 日志。
    ...
    def __init__(self, parent=None):
        ...
        # Likert 标签
        self.likert_labels_3 = {1: "不同意", 2: "一般", 3: "同意"}
        self.likert_labels_5 = {1: "非常不同意", 2: "不同意", 3: "一般", 4: "同意", 5: "非常同意"}
    ...
    def on_start_clicked(self):
        ...
        self._show_fullscreen_message(
            "{n}秒后将开始实验", self.initial_countdown, plain=True, next_callback=self._start_next_trial
        )
    ...
    def _start_next_trial(self):
        ...
        if self.trial_index >= len(self.trial_plan):
            ...
            self._show_fullscreen_message(
                "{n}秒后实验结束", self.end_countdown, plain=True, next_callback=self._finish_and_save
            )
            return
        ...
        prompt_ms = int(self.prompt_duration * 1000)
        task_sec = int(
            random.randint(min(int(self.task_min), int(self.task_max)), max(int(self.task_min), int(self.task_max)))
        )
        task_ms = task_sec * 1000
        assess_ms = int(self.assess_duration * 1000)
        break_ms = int(self.break_duration * 1000)
