...
class Page6Widget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.initial_countdown = 10  # 实验开始前 & 实验结束前倒计时（秒）
        self.rest_duration = 180      # 循环间休息倒计时（秒）
    ...
    def _build_main_ui(self):
        ...
        # ====== 新增：开始/结束倒计时、休息倒计时（两行） ======
        self.start_end_countdown_spin = QSpinBox()
        self.start_end_countdown_spin.setRange(0, 3600)
        self.start_end_countdown_spin.setValue(int(self.initial_countdown))
        form.addRow("Start/End Countdown (s):", self.start_end_countdown_spin)

        self.rest_countdown_spin = QSpinBox()
        self.rest_countdown_spin.setRange(0, 36000)
        self.rest_countdown_spin.setValue(int(self.rest_duration))
        form.addRow("Rest Countdown (s):", self.rest_countdown_spin)
        # =======================================================
    ...
    def on_start_clicked(self):
        ...
        # ====== 新增：读取倒计时参数 ======
        self.initial_countdown = int(self.start_end_countdown_spin.value())
        self.rest_duration = int(self.rest_countdown_spin.value())
        # =================================
        ...
        try:
            with open(fname, 'w', encoding='utf-8') as f:
                for i in range(self.loops):
                    ...
                    start_val = float(start_t) if isinstance(start_t, (int, float)) else float('nan')
                    end_val = float(end_t) if isinstance(end_t, (int, float)) else float('nan')
    ...
    def _save_meta_json(self, aborted: bool = False):
        ...
        for i in range(self.loops):
            ...
            start_val = float(start_t) if isinstance(start_t, (int, float)) else None
            end_val = float(end_t) if isinstance(end_t, (int, float)) else None
    ...
    def _show_fullscreen_message(
        ...
        # seconds <= 0：直接进入下一步（避免多等 1 秒）
        if seconds <= 0:
            self.countdown_label.hide()
            if callable(next_callback):
                QtCore.QTimer.singleShot(0, next_callback)
            return
