...
#       其中：
#         - start_time：该半周期从“最黑”开始的时间（秒）（校准电脑时间，黑→白为 stim_start + n*T，白→黑为 stim_start + n*T + T/2）；
#         - end_time：该半周期到达另一极值时刻的时间（秒）（等于 start_time + T/2）；
#         - label：为 "black_to_white" 或 "white_to_black"。
#       例如：
#         10 个 trial，freq=0.5 Hz，stim=6 秒 → 每个 trial 有 3 个周期 →
#         每周期 2 个半周期 → 总共 60 行。
#     """
...
#         # 刺激期不再隐藏文字标签，只清空文本，保证底部高度恒定，注视点不移动
#         self.message_label.show()
...
#           - start_time：半周期的开始时间（秒），黑→白为 stim_start + n*T，白→黑为 stim_start + n*T + T/2；
#           - end_time：半周期结束时间（秒），start_time + T/2；
#           - label："black_to_white" 或 "white_to_black"。

#         对每个完整周期，依次写入两行：
#           1) 黑到白 half-cycle:  black_to_white
#           2) 白到黑 half-cycle:  white_to_black

#         cycles = floor(stim_duration * freq)，每个 trial 产生 2 * cycles 行。
#         """
...
#                     # cycles 取 floor，保证是完整周期数（如 0.5Hz、6s→3 个周期）
...
#             # 全屏窗口里再加一个 ESC 快捷键，保证焦点在全屏时也能中断
...
#             # 关键：直接 showFullScreen，而不是 setWindowState(WindowFullScreen)+show
...
#         """
#         关闭全屏窗口，并把 screen_container 放回 Page11（但不加入布局）。
#         """
